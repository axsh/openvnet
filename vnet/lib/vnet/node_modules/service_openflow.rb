  ...
  module NodeModules
    ...
    class ServiceOpenflow
      ...
      def start
        ...
        dispatch_event("vna_initialized",
                       node: "#{conf.node.id}",
                       actor: "#{conf.node.id}-#{conf.node.addr.host}")
      end
      ...
      def update_topology(network_id, dpid)
        debug "update_topology: network_id => #{network_id}, dpid => #{dpid}"
        @controller.update_topology(dpid, network_id)
      end

      def released_ip(params)
        @controller.datapaths.first.last.interface_manager.async.handle_event(params)
      end

      def handle_event(event, options)
        debug "handle event: #{event} options: #{options.inspect}"
