  ...
  module NodeModules
    class ServiceWatchdog
      ...
      def handle_completed(id, entry, value)
        ...
        if print_log
          debug log_format_h('handling completed check',
                             id: value[:id],
                             event_task_count: value[:event_task_count],
                             event_task_stuck: value[:event_task_stuck] && value[:event_task_stuck].size)
        end

        if print_log && value[:event_task_stuck] && !value[:event_task_stuck].empty?
          value[:event_task_stuck].each { |stuck_task|
            debug log_format_h('stuck event task', stuck_task)
          }
        end
      end
