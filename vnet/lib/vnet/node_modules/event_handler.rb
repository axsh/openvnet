  ...
  class EventHandler
    ...
    def handle_event(event, options = {})
      ...
      when ADDED_DATAPATH_NETWORK
        ...
        datapath_network.batch.datapath_networks_in_the_same_network.commit(fill: :datapath).each do |dpn|
          publish_event(dpn.datapath.node_id, event, options)
        end
      when REMOVED_DATAPATH_NETWORK
        datapath_network = api.datapath_network.only_deleted.first(id: options[:datapath_network_id])
      ...
      when REMOVED_DATAPATH
        node_id = api.datapath.only_deleted.first(id: options[:id]).node_id
        publish_event(node_id, event, options)
      else
