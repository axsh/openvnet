...
module Vnet::NodeModules
  class EventHandler
    ...
    def handle_event(event, options = {})
      ...
      when "network/iface_added"
        api.datapath.find_all_by_network_id(options[:network_id]).each do |datapath|
          debug "node_id: #{datapath[:node_id]}"
          # TODO implement me
          # ex) DCell::Node[datapath.node_id][:api].iface_added(options[:network_id], options[:iface_id])
        end
      when "network/iface_removed"
        api.datapath.find_all_by_network_id(options[:network_id]).each do |datapath|
          debug "node_id: #{datapath[:node_id]}"
          # TODO implement me
          # ex) DCell::Node[datapath.node_id][:api].iface_removed(options[:network_id], options[:iface_id])
        end
      end
    end
  end
