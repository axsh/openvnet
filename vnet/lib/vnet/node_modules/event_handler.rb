  ...
  class EventHandler
    ...
    def handle_event(event, options = {})
      ...
      when ADDED_DATAPATH_NETWORK
        datapath_network = api.datapath_network[options[:datapath_network_id]]
      ...
      when REMOVED_DATAPATH_NETWORK
        datapath_network = api.datapath_network.with_deleted.first(id: options[:datapath_network_id])
    ...
    #TODO refactor
    def test_method(klass, to_hash)
      h = {}
      h[:uuid] = 'nw-vnettest'
      h[:display_name] = to_hash[:display_name]
      h[:ipv4_network] = to_hash[:ipv4_network]
      h[:ipv4_prefix] = to_hash[:prefix]
      h[:domain_name] = to_hash[:domain_name]
      h[:network_mode] = 'virtual'
      h[:editable] = to_hash[:editable]
      Vnet::ModelWrappers::Network.create(h)
    end
