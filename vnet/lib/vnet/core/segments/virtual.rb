...
module Vnet::Core::Segments
  ...
  class Virtual < Base
    ...
    def install
      # segment_md = md_create(:segment => @id)
      # fo_segment_md = flow_options.merge(segment_md)

      flows = []
      # flows << Flow.create(TABLE_TUNNEL_SEGMENT_IDS, 30, {
      #                        :tunnel_id => @id | TUNNEL_FLAG_MASK
      #                      }, nil,
      #                      fo_segment_md.merge(:goto_table => TABLE_SEGMENT_SRC_CLASSIFIER))
    ...
    def create_ovs_flow_learn_arp(priority, match_options = "", learn_options = "")
      ...
      #
      # match_md = md_create(segment: @id, remote: nil)
