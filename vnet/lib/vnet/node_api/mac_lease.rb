  ...
  class MacLease < Base
    class << self
      def create(options)
        ...
        dispatch_event(LEASED_MAC_ADDRESS, target_id: mac_lease.interface_id, mac_lease_id: mac_lease.id)
      ...
      def update(uuid, options)
        ...
        if deleted_mac_address
          dispatch_event(RELEASED_MAC_ADDRESS, target_id: mac_lease.interface_id, mac_lease_id: mac_lease.id)
          dispatch_event(LEASED_MAC_ADDRESS, target_id: mac_lease.interface_id, mac_lease_id: mac_lease.id)
        end
