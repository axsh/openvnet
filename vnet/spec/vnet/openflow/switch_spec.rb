  ...
  describe "handle_port_desc" do
    context "tunnel" do
      it "should create a port object whose datapath_id is 1" do
        ...
        allow_any_instance_of(Vnet::Openflow::TunnelManager).to receive(:item).and_return(tunnel)

        dp.create_mock_port_manager
        port_desc = double(:port_desc)
        allow(port_desc).to receive(:port_no).and_return(5)
        allow(port_desc).to receive(:name).exactly(1).times.and_return('t-a')
        allow(port_desc).to receive(:hw_addr).exactly(1).times.and_return(nil)
