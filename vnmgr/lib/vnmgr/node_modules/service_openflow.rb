...
module Vnmgr
  module NodeModules
    ...
    class ServiceOpenflow
      ...
      def controller
        @controller ||= Vnmgr::VNet::Openflow::Controller.new(self)
      end

      def create_tunnels
        all_dps = Vnmgr::ModelWrappers::Datapath.all
        mydp = all_dps.detect { |dp| dp.node_id == conf.node.id }

        ofctl = Vnmgr::VNet::Openflow::OvsOfctl.new
        all_dps.each do |dp|
          next if dp.node_id == mydp.node_id
          ofctl.add_gre_tunnel("t-#{dp.node_id}", IPAddr.new(dp.ipv4_address, Socket::AF_INET).to_s)
          #exec("ovs-vsctl add-port #{conf.bridge_name} t-#{dp.node_id} -- set interface type=gre options:remote_ip=#{dp.ipv4_address}")
          Vnmgr::ModelWrappers::Tunnel.create(:src_datapath_id => mydp.id, :dst_datapath_id => dp.id)
        end
      end

      def conf
        Vnmgr::Configurations::Vna.conf
      end
    end
