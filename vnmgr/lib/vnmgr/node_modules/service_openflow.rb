  ...
  module NodeModules
    ...
    class ServiceOpenflow
      ...
      def create_tunnels
        ...
        ofctl = Vnmgr::VNet::Openflow::OvsOfctl.new(mydp.dpid)
        all_dps.each do |dp|
          next if dp.node_id == mydp.node_id
          next if dp.dc_segment_id == mydp.dc_segment_id
          tunnel = Vnmgr::ModelWrappers::Tunnel.create(:src_datapath_id => mydp.id, :dst_datapath_id => dp.id)
          ofctl.add_gre_tunnel(tunnel.uuid, IPAddr.new(dp.ipv4_address, Socket::AF_INET).to_s)
        end
      end
