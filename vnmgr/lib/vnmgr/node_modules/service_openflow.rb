...
module Vnmgr
  module NodeModules
    ...
    class ServiceOpenflow
      ...
      def create_tunnels
        mydp = Vnmgr::ModelWrappers::Datapath.first(:node_id => Vnmgr::Configurations::Vna.conf.node.id)
        ofctl = Vnmgr::VNet::Openflow::OvsOfctl.new(mydp.dpid)

        Vnmgr::ModelWrappers::Datapath.batch.where(~{:node_id => mydp.node_id}).where(~{:dc_segment_id => mydp.dc_segment_id}).all.commit.each do |other_dp|
          tunnel = Vnmgr::ModelWrappers::Tunnel.create(:src_datapath_id => mydp.id, :dst_datapath_id => other_dp.id)
          ofctl.add_tunnel(tunnel.uuid, IPAddr.new(other_dp.ipv4_address, Socket::AF_INET).to_s)
        end
      end
    end
