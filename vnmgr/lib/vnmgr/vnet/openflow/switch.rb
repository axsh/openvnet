  ...
  class Switch
    ...
    def handle_port_desc(port_desc)
      ...
      if port.port_number >= OFPP_LOCAL
        ...
        port_map = Vnmgr::ModelWrappers::Vif[port_desc.name]
        ...
        # network = self.network_manager.network_by_id(port_map.network_id)
        network = self.network_manager.network_by_uuid(port_map.batch.network.commit.uuid)
