...
Vnmgr::Endpoints::V10::VNetAPI.namespace '/ip_leases' do
  ...
  post do
    ...
    ip_lease = data_access.ip_lease.create(params)
    respond_with(R::IpLease.generate(ip_lease))
  end

  get do
    ip_leases = data_access.ip_lease.all
    respond_with(R::IpLeaseCollection.generate(ip_leases))
  end

  get '/:uuid' do
    ip_lease = data_access.ip_lease[{:uuid => @params["uuid"]}]
    respond_with(R::IpLease.generate(ip_lease))
  end

  delete '/:uuid' do
    ip_lease = data_access.ip_lease.delete({:uuid => @params["uuid"]})
    respond_with(R::IpLease.generate(ip_lease))
  end

  put '/:uuid' do
    params = parse_params(@params, ["uuid","network_uuid","vif_uuid","ip_address_uuid","alloc_type","is_deleted","created_at","updated_at","deleted_at"])
    ip_lease = data_access.ip_lease.update(params)
    respond_with(R::IpLease.generate(ip_lease))
  end
end
