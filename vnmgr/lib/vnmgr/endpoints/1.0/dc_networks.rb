...
Vnmgr::Endpoints::V10::VNetAPI.namespace '/dc_networks' do
  post do
    ...
    dc_network = data_access.dc_network.create(params)
    respond_with(R::DcNetwork.generate(dc_network))
  end

  get do
    dc_networks = data_access.dc_network.all
    respond_with(R::DcNetworkCollection.generate(dc_networks))
  end

  get '/:uuid' do
    dc_network = data_access.dc_network[{:uuid => @params["uuid"]}]
    respond_with(R::DcNetwork.generate(dc_network))
  end

  delete '/:uuid' do
    dc_network = data_access.dc_network.delete({:uuid => @params["uuid"]})
    respond_with(R::DcNetwork.generate(dc_network))
  end

  put '/:uuid' do
    params = parse_params(@params, ["uuid","display_name","parent_id","created_at","updated_at"])
    dc_network = data_access.dc_network.update(new_params)
    respond_with(R::DcNetwork.generate(dc_network))
  end
end
