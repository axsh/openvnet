...
Vnmgr::Endpoints::V10::VNetAPI.namespace '/network_services' do
  ...
  post do
    ...
    network_service = data_access.network_service.create(params)
    respond_with(R::NetworkService.generate(network_service))
  end

  get do
    network_services = data_access.network_service.all
    respond_with(R::NetworkServiceCollection.generate(network_services))
  end

  get '/:uuid' do
    network_service = data_access.network_service[{:uuid => @params["uuid"]}]
    respond_with(R::NetworkService.generate(network_service))
  end

  delete '/:uuid' do
    network_service = data_access.network_service.delete({:uuid => @params["uuid"]})
    respond_with(R::NetworkService.generate(network_service))
  end

  put '/:uuid' do
    params = parse_params(@params, ["uuid","vif_uuid","display_name","incoming_port","outgoing_port","created_at","updated_at"])
    network_service = data_access.network_service.update(params)
    respond_with(R::NetworkService.generate(network_service))
  end
end
