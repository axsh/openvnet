...
openssl req -new -nodes -text -out "${root_csr}" -keyout "${root_key}" -subj /CN=${domain}
...
openssl req -new -nodes -text -out "${server_csr}" -keyout "${server_key}" -subj /CN=${domain}
...
openssl req -new -nodes -text -out "${client_csr}" -keyout "${client_key}" -subj /CN=${domain}
